...
(define (PrintRow deg precision)
  (display "| ")
  (display (number->string (round-to deg 2)))
  (display " | ")
  (let ((sin-value (sin (* (/ pi 180) deg)))
        (cos-value (cos (* (/ pi 180) deg)))
        (tan-value (tan (* (/ pi 180) deg))))
    (display (number->string (round-to sin-value precision)))
    (display " | ")
    (display (number->string (round-to cos-value precision)))
    (display " | ")
    (if (= (round-to cos-value precision) 0)
        (display "    inf   ")
        (display (number->string (round-to tan-value precision))))
    (display " |"))
  (newline))
